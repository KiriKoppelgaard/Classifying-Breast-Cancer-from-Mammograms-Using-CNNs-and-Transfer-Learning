...
for file in filenames:
    image, label = read_data(file, transfer_learning=False) ### keeping it false it check whether we really need rgb for tl 
    images.append(image)
    labels.append(label)
...
x_val = x_test.reshape(x_val.shape[0], x_val.shape[1], x_val.shape[2], 1)
...
inceptionv3 = InceptionV3(
    ...
    weights="imagenet", # include pre-trained weights from training on imagenet; we could also try to include a version without? 
    input_tensor=None,
 ...
 #output layer
#inceptionv3.add(Dense(2, activation='sigmoid')) #feed-forward layer with softmax
